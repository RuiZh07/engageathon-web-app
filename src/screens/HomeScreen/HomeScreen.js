...
import './HomeScreen.scss'; // Import the SCSS file
...
export default function HomeScreen() {
    ...
    useEffect(() => {
        const fetchEventList = async () => {
            try {
                const response = await fetch(`http://app.engageathon.com/api/eventslist/`, {
                    method: 'GET',
                    headers: {
            ...
            {eventList.map((post) => (
                <div key={post.id} className="homePost">
                    <div className="postHeader">
                        <img
                            src={post.profile_photo_url ? `data:image/jpeg;base64,${post.profile_photo_url}` : ""}  
                            alt="Organizer"
