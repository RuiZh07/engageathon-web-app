...
export default function SuperBowlActivity() {
    ...
    // Split activites into two columns for UI rendering
    ...
    const fetchActivities = async (email) => {
        try {
            ...
            const response = await fetch(
                `https://app.engageathon.com/api/events/activity/9/${email}/`, 
                {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        if (location.state && location.state.completedActivityId) {
            ...
            // Find the activity that matched the completedActivityId
    ...
    const setActivityConfirmed = async (token) => {
        ...
        if (userData) {
            ...
            try {
                const response = await fetch(
                    `https://app.engageathon.com/api/events/activity/scan/9/${activeActivity?.activity}/`,
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                ...
                if (response.ok) {
                    ...
                    navigate("/congrats-activity", {
                        state: {
                            activityId: activeActivity.activity,
                        ...
                        <button key={activity.id} className="activityContainer" onClick={() => handleActivityPress(activity)}>
                            <div className="iconsContainer">
                                {activity.activity === 95 && (
                                    <div className="superbowlIconBackground">
                                        <img src={sipSmoke} className="superbowlIcon" alt="sip smoke" />
                                    </div>
                                )}
                                {activity.activity === 96 && (
                                    <div className="superbowlIconBackground">
                                        <img src={masquerade} className="superbowlIcon" alt="masquerade" />
                                    </div>
                                )}
                                {activity.activity === 97 && (
                                    <div className="superbowlIconBackground">
                                        <img src={bigGame} className="superbowlIcon" alt="big game" />
                                    </div>
                                )}
